...
mod tests {
    ...
    /// Diagnostic: reproduce the seccomp EPERM that nu hits when loading
    /// config via `--config` under lot's sandbox. Spawns nu with --config,
    /// RUST_BACKTRACE=1, captures stderr, and intentionally panics to dump
    /// the full backtrace in CI output.
    ///
    /// This test only runs on Linux (other platforms don't use seccomp).
    /// Remove once lot adds the missing prctl/ioctl to its allowlist.
    #[tokio::test]
    #[cfg(target_os = "linux")]
    async fn diagnostic_config_loading_seccomp_backtrace() {
        skip_no_nu!();
        let project = tmp_sandbox_project();
        let tool = tmp_sandbox_tool_dir();
        ...
        let temp_base = project.path().join(".reel").join("tmp");
        std::fs::create_dir_all(&temp_base).unwrap();
        let session_temp_dir = tempfile::TempDir::new_in(&temp_base).unwrap();
        ...
        let nu_binary = resolve_nu_binary(Some(tool.path()));
        let config_files = resolve_config_files(Some(tool.path()));
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            // Pass --config to trigger the problematic code path
            ...
            let mut child = lot::spawn(&policy, &cmd).map_err(|e| format!("spawn failed: {e}"))?;

            let stderr_handle = child.take_stderr().ok_or("no stderr")?;
            let stdin = child.take_stdin().ok_or("no stdin")?;
            drop(stdin); // close stdin to let nu exit

            // Give nu time to attempt config loading and panic
            std::thread::sleep(std::time::Duration::from_secs(2));

            let mut stderr_content = String::new();
            use std::io::Read;
            // Read available stderr (non-blocking would be ideal but
            // simple blocking read with a killed process is fine)
            let _ = child.kill();
            let stderr_file: std::fs::File = stderr_handle;
            let mut reader = std::io::BufReader::new(stderr_file);
            let _ = reader.read_to_string(&mut stderr_content);

            Ok::<String, String>(stderr_content)
        })
        .await
        .unwrap();

        let stderr = result.unwrap_or_else(|e| format!("spawn error: {e}"));
        // Intentional panic to surface the full stderr in CI logs
        panic!(
            "SECCOMP DIAGNOSTIC — nu stderr with RUST_BACKTRACE=full:\n{}",
            stderr
        );
    }
